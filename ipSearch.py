...
def search_asn(inputted_ip_addr):
    ...
    while low <= high:
        ...
        if inputted_ip_addr >= range_start and inputted_ip_addr <= range_end:
            ip_start = splitted_line[0]
            ip_end = splitted_line[1]
    ...
    return ip_start, ip_end, as_number, as_description, country_code
...
def main():
    ...
    # ----- Search the correct ASN by Binary Search -----
    ip_start, ip_end, as_number, as_description, country_code = search_asn(inputted_ip_addr)

    # ----- Display the reuslt of ASN search -----
    print(format(ip_start),format(ip_end),format(country_code),format(as_number),format(as_description))
